...
app.get("/all", (req, res) => {
  ...
  Promise.all([p1, p2]).then((results) => {
    ...
    equality_list.forEach((equality) => {
      let table_row = "<tr>";
      table_row += `<td>${equality.country}</td>`;
      table_row += `<td>${equality.wei}</td>`;
      table_row += `<td>${equality.weg}</td>`;
      table_row += `<td>${equality.ggpi}</td>`;
      table_row += `<td>${equality.ggpg}</td>`;
      table_row += `<td>${equality.hdg}</td>`;
      table_row += `<td>${equality.region}</td>`;
      table_row += "</tr>";
